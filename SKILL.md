---
name: hyperplan
description: "Adversarial multi-agent planning skill. Self-orchestrates 5 hostile agents (skeptic, validator, researcher, architect, creative) via parallel Agent-tool dispatch for ruthless cross-critique debate (3 rounds). Agents exchange findings through a filesystem message bus (.cache/phase_N/<member>.md) — the Lead never forwards bundles. After 3 rounds the Lead distills survivors and MANDATORILY hands the insight bundle to the `Plan` agent for executable plan formalization. Use when planning needs maximum rigor and surfacing of weak assumptions, blind spots, and over-engineering. Triggers: 'hyperplan', 'hpp', '/hyperplan', 'adversarial plan', 'hostile planning', 'cross-critique plan'."
---
...
You (the orchestrator) become the **Lead** of a 5-member adversarial team. The 5 members are **maximally hostile** to each other — they attack each other's findings ruthlessly. Members exchange their outputs through a **filesystem message bus** (`.cache/phase_N/<member>.md`): each member writes its own output file, and reads the other members' files directly. The Lead never forwards bundles — the Lead only dispatches, waits for completion, and finally reads `.cache/phase_3/` to distill survivors into an insight bundle, which is MANDATORILY handed to the `Plan` agent.
...
## FILESYSTEM MESSAGE BUS (.cache/)

Members do NOT send messages to each other through the Lead. Instead, every member writes its round output to a fixed path, and members read each other's files directly in the next round.
...
- `.cache/` is gitignored runtime state. It is left in place after the run for inspection; the user may delete it.
- The Write tool auto-creates these directories on first write — no need to mkdir.
- The Lead reads `.cache/phase_3/*.md` (plus `phase_1/` and `phase_2/` for provenance) ONLY in Phase 4 for distillation. Phases 1–3 never put member content into the Lead's context.
...
- **Wait for a resumed member to finish writing**: `TaskOutput` with `block: true` against the member's task id. You do NOT need the returned content — the canonical output is the file the agent wrote to `.cache/`. TaskOutput is just the completion gate.
...
4. Resolve the absolute path to this skill's directory (`<skill_dir>` — where SKILL.md lives). All member system prompts, task templates, and `.cache/` paths derive from it.
...
1. Read `references/round-1-task.md`. For each member, substitute:
   ...
   - `{{OUTPUT_PATH}}` → `<skill_dir>/.cache/phase_1/<member>.md`
...
3. Capture each member's returned identifier (description / agentId) — you need it to resume them in Round 2. Do NOT read the `.cache/phase_1/` files yourself; you do not need them yet.

### Phase 2: Round 2 — Cross-attack (read phase_1, write phase_2)

1. Read `references/round-2-task.md`. For each member, substitute:
   - `{{INPUT_DIR}}` → `<skill_dir>/.cache/phase_1`
   - `{{MEMBER_NAME}}` → the member's name
   - `{{OUTPUT_PATH}}` → `<skill_dir>/.cache/phase_2/<member>.md`
...
3. Issue 5 `TaskOutput` calls in a single message (one per member, `block: true`) to wait for all 5 to finish writing. The returned content is just a completion signal — the canonical output is on disk in `.cache/phase_2/`.
...
1. Read `references/round-3-task.md`. For each member, substitute:
   - `{{INPUT_DIR}}` → `<skill_dir>/.cache/phase_2`
   - `{{MEMBER_NAME}}` → the member's name
   - `{{OUTPUT_PATH}}` → `<skill_dir>/.cache/phase_3/<member>.md`
...
The team is done debating. Your job at this phase is **distillation only** — you do NOT write the work plan. You read the `.cache/` artifacts and produce a structured insight bundle that the `Plan` agent will consume in Phase 5.

1. **Read the artifacts**:
   - `.cache/phase_1/<member>.md` for all 5 members — the original Round 1 findings.
   - `.cache/phase_2/<member>.md` for all 5 members — the cross-attacks (to see what was contested, and by whom).
   - `.cache/phase_3/<member>.md` for all 5 members — the defenses (DEFEND / REFINE / CONCEDE per finding).
...
DO NOT save the plan to disk unless the user asks. Hyperplan is a planning consultation, not a file-emitting workflow — the plan lives in your conversation output. (The `.cache/` artifacts are debate transcripts, not the deliverable.)
...
2. Leave `.cache/` in place for the user to inspect the debate transcript if desired. Mention its location. The user may delete it — it is gitignored.
...
| **Lead forwarding bundles between members instead of using the `.cache/` message bus** | **Defeats the filesystem-bus design. Members read each other's files directly from `.cache/phase_N/`. The Lead's context must stay clean — never paste member output into a `SendMessage`.** |
| **Lead reading `.cache/` files before Phase 4** | **Unnecessary context pressure. Phases 1–3 only need the completion signal from `TaskOutput`, not the content.** |
...
- The orchestrator Reads `.cache/` artifacts ONLY in Phase 4. Phases 1–3 coordinate timing only — member content flows member-to-member through the filesystem, never through the Lead.
...
- The Plan agent does NOT have access to the member agents or to `.cache/`. Everything it needs must be in the handoff prompt (the distilled insight bundle). If the planner asks for additional context, you fetch it (via Read on `.cache/` or Explore) and re-dispatch with a `SendMessage` to the Plan agent's task id — do NOT spin up a new Plan agent.
